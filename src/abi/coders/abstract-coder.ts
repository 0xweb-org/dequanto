...
export class Writer {
    ...
    get data(): string {
        return $buffer.toHex($buffer.concat(this.#data));
    }
    ...
    writeBytes(value: Uint8Array): number {
        ...
        if (paddingOffset) {
            bytes = $buffer.concat([ bytes, Padding.slice(paddingOffset) ]);
        }
...
export class Reader {
    ...
    constructor(data: string | Uint8Array, allowLoose?: boolean) {
        ...
        this.#data = $buffer.ensure(data);
        this.#offset = 0;
    }

    get data(): string { return $buffer.toHex(this.#data); }
    ...
    readIndex(): number {
        return Number($buffer.toBigInt(this.readBytes(WordSize)));
    }
}
