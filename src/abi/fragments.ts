        ...
        if (type) {
            ...
            if (type === "OPEN_PAREN") {
                ...
                if (tokens.length > 0 && tokens[tokens.length - 1].type === "NUMBER") {
                    ...
                    suffix = value + suffix;
                    (<Writeable<Token>>(tokens[tokens.length - 1])).value = Number(value);
                }
...
export class FallbackFragment extends Fragment {
    ...
    static from(obj: any): FallbackFragment {
        ...
        if (typeof(obj) === "string") {
            ...
            if (type === "receive") {
                const inputs = consumeParams(obj);
                $require.True(inputs.length === 0, `receive cannot have arguments ` + inputs);
            ...
            if (inputs.length) {
                $require.True(inputs.length === 1 && inputs[0].type === "bytes",
            ...
            $require.True(mutability === "nonpayable" || mutability === "payable", "fallback cannot be constants " + mutability);

            if (consumeKeywords(obj, setify([ "returns" ])).has("returns")) {
                const outputs = consumeParams(obj);
                $require.True(outputs.length === 1 && outputs[0].type === "bytes",
