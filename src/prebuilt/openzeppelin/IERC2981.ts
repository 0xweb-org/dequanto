...
export class IERC2981 extends ContractBase {
    ...
    Types: TIERC2981Types;

    $meta = {
        "class": "./src/prebuilt/openzeppelin/IERC2981.ts"
    }
    ...
    $signed (): TOverrideReturns<IIERC2981TxCaller, Promise<{ signed: TEth.Hex, error?: Error & { data?: { type: string, params } } }>> {
        return super.$signed() as any;
    }
    $data (): IIERC2981TxData {
        return super.$data() as IIERC2981TxData;
    }
    $gas (): TOverrideReturns<IIERC2981TxCaller, Promise<{ gas?: bigint, price?: bigint, error?: Error & { data?: { type: string, params } } }>> {
        return super.$gas() as any;
    }

    onTransaction <TMethod extends keyof TIERC2981Types['Methods']> (method: TMethod, options: Parameters<ContractBase['$onTransaction']>[0]): SubjectStream<{
        tx: TEth.Tx
        block: TEth.Block<TEth.Hex>
        calldata: {
            method: TMethod
            arguments: TIERC2981Types['Methods'][TMethod]['arguments']
        }
    }> {
        options ??= {};
        options.filter ??= {};
...
type TEvents = TIERC2981Types['Events'];
