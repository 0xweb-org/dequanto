...
export class ERC20Capped extends ContractBase {
    ...
    Types: TERC20CappedTypes;

    $meta = {
        "class": "./src/prebuilt/openzeppelin/ERC20Capped.ts"
    }
    ...
    $signed (): TOverrideReturns<IERC20CappedTxCaller, Promise<{ signed: TEth.Hex, error?: Error & { data?: { type: string, params } } }>> {
        return super.$signed() as any;
    }
    $data (): IERC20CappedTxData {
        return super.$data() as IERC20CappedTxData;
    }
    $gas (): TOverrideReturns<IERC20CappedTxCaller, Promise<{ gas?: bigint, price?: bigint, error?: Error & { data?: { type: string, params } } }>> {
        return super.$gas() as any;
    }

    onTransaction <TMethod extends keyof TERC20CappedTypes['Methods']> (method: TMethod, options: Parameters<ContractBase['$onTransaction']>[0]): SubjectStream<{
        tx: TEth.Tx
        block: TEth.Block<TEth.Hex>
        calldata: {
            method: TMethod
            arguments: TERC20CappedTypes['Methods'][TMethod]['arguments']
        }
    }> {
        options ??= {};
        options.filter ??= {};
...
type TEvents = TERC20CappedTypes['Events'];
