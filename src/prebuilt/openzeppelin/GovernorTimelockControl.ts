...
export class GovernorTimelockControl extends ContractBase {
    ...
    Types: TGovernorTimelockControlTypes;

    $meta = {
        "class": "./src/prebuilt/openzeppelin/GovernorTimelockControl.ts"
    }
    ...
    $signed (): TOverrideReturns<IGovernorTimelockControlTxCaller, Promise<{ signed: TEth.Hex, error?: Error & { data?: { type: string, params } } }>> {
        return super.$signed() as any;
    }
    $data (): IGovernorTimelockControlTxData {
        return super.$data() as IGovernorTimelockControlTxData;
    }
    $gas (): TOverrideReturns<IGovernorTimelockControlTxCaller, Promise<{ gas?: bigint, price?: bigint, error?: Error & { data?: { type: string, params } } }>> {
        return super.$gas() as any;
    }

    onTransaction <TMethod extends keyof TGovernorTimelockControlTypes['Methods']> (method: TMethod, options: Parameters<ContractBase['$onTransaction']>[0]): SubjectStream<{
        tx: TEth.Tx
        block: TEth.Block<TEth.Hex>
        calldata: {
            method: TMethod
            arguments: TGovernorTimelockControlTypes['Methods'][TMethod]['arguments']
        }
    }> {
        options ??= {};
        options.filter ??= {};
...
export type TGovernorTimelockControlTypes = {
    Events: {
        EIP712DomainChanged: {
            outputParams: {  },
            outputArgs:   [  ],
...
type TEvents = TGovernorTimelockControlTypes['Events'];
