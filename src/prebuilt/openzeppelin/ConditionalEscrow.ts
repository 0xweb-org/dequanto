...
export class ConditionalEscrow extends ContractBase {
    ...
    $meta = {
        "class": "./src/prebuilt/openzeppelin/ConditionalEscrow.ts"
    }
    ...
    async withdrawalAllowed (payee: TAddress): Promise<boolean> {
        return this.$read(this.$getAbiItem('function', 'withdrawalAllowed'), payee);
    }

    $call () {
        return super.$call() as IConditionalEscrowTxCaller;
    }
    $signed (): TOverrideReturns<IConditionalEscrowTxCaller, Promise<{ signed: TEth.Hex, error?: Error & { data?: { type: string, params } } }>> {
        return super.$signed() as any;
    }
    $data (): IConditionalEscrowTxData {
        return super.$data() as IConditionalEscrowTxData;
    }
    $gas (): TOverrideReturns<IConditionalEscrowTxCaller, Promise<{ gas?: bigint, price?: bigint, error?: Error & { data?: { type: string, params } } }>> {
        return super.$gas() as any;
    }

    onTransaction <TMethod extends keyof TConditionalEscrowTypes['Methods']> (method: TMethod, options: Parameters<ContractBase['$onTransaction']>[0]): SubjectStream<{
        tx: TEth.Tx
        block: TEth.Block<TEth.Hex>
        calldata: {
            method: TMethod
            arguments: TConditionalEscrowTypes['Methods'][TMethod]['arguments']
        }
    }> {
        options ??= {};
        options.filter ??= {};
...
export type TConditionalEscrowTypes = {
    Events: {
        Deposited: {
...
interface IConditionalEscrowTxCaller {
...
interface IConditionalEscrowTxData {
