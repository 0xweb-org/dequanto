...
export class GovernorTimelockCompound extends ContractBase {
    ...
    Types: TGovernorTimelockCompoundTypes;

    $meta = {
        "class": "./src/prebuilt/openzeppelin/GovernorTimelockCompound.ts"
    }
    ...
    $signed (): TOverrideReturns<IGovernorTimelockCompoundTxCaller, Promise<{ signed: TEth.Hex, error?: Error & { data?: { type: string, params } } }>> {
        return super.$signed() as any;
    }
    $data (): IGovernorTimelockCompoundTxData {
        return super.$data() as IGovernorTimelockCompoundTxData;
    }
    $gas (): TOverrideReturns<IGovernorTimelockCompoundTxCaller, Promise<{ gas?: bigint, price?: bigint, error?: Error & { data?: { type: string, params } } }>> {
        return super.$gas() as any;
    }

    onTransaction <TMethod extends keyof TGovernorTimelockCompoundTypes['Methods']> (method: TMethod, options: Parameters<ContractBase['$onTransaction']>[0]): SubjectStream<{
        tx: TEth.Tx
        block: TEth.Block<TEth.Hex>
        calldata: {
            method: TMethod
            arguments: TGovernorTimelockCompoundTypes['Methods'][TMethod]['arguments']
        }
    }> {
        options ??= {};
        options.filter ??= {};
    ...
    onTimelockChange (fn?: (event: TClientEventsStreamData<TEventArguments<'TimelockChange'>>) => void): ClientEventsStream<TClientEventsStreamData<TEventArguments<'TimelockChange'>>> {
    ...
    extractLogsTimelockChange (tx: TEth.TxReceipt): ITxLogItem<TEventParams<'TimelockChange'>>[] {
        let abi = this.$getAbiItem('event', 'TimelockChange');
        return this.$extractLogs(tx, abi) as any as ITxLogItem<TEventParams<'TimelockChange'>>[];
    }
    ...
    async getPastLogsTimelockChange (options?: {
        ...
        toBlock?: number | Date
        params?: {  }
    }): Promise<ITxLogItem<TEventParams<'TimelockChange'>>[]> {
...
export type TGovernorTimelockCompoundTypes = {
    Events: {
        EIP712DomainChanged: {
            outputParams: {  },
            outputArgs:   [  ],
        ...
        TimelockChange: {
            outputParams: { oldTimelock: TAddress, newTimelock: TAddress },
            outputArgs:   [ oldTimelock: TAddress, newTimelock: TAddress ],
        }
        VoteCast: {
...
interface IGovernorTimelockCompoundTxData {
    __acceptAdmin (sender: TSender, ): Promise<TEth.TxLike>
...
type TEvents = TGovernorTimelockCompoundTypes['Events'];
