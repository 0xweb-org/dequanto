...
export class ERC1155 extends ContractBase {
    constructor(
    ...
    $meta = {
        "class": "./src/prebuilt/openzeppelin/ERC1155.ts"
    }

    async $constructor (deployer: TSender, uri_: string): Promise<TxWriter> {
    ...
    $call () {
        return super.$call() as IERC1155TxCaller;
    }
    $signed (): TOverrideReturns<IERC1155TxCaller, Promise<{ signed: TEth.Hex, error?: Error & { data?: { type: string, params } } }>> {
        return super.$signed() as any;
    }
    $data (): IERC1155TxData {
        return super.$data() as IERC1155TxData;
    }
    $gas (): TOverrideReturns<IERC1155TxCaller, Promise<{ gas?: bigint, price?: bigint, error?: Error & { data?: { type: string, params } } }>> {
        return super.$gas() as any;
    }

    onTransaction <TMethod extends keyof TERC1155Types['Methods']> (method: TMethod, options: Parameters<ContractBase['$onTransaction']>[0]): SubjectStream<{
        tx: TEth.Tx
        block: TEth.Block<TEth.Hex>
        calldata: {
            method: TMethod
            arguments: TERC1155Types['Methods'][TMethod]['arguments']
        }
    }> {
        options ??= {};
        options.filter ??= {};
