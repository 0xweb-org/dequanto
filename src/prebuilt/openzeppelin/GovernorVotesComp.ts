...
export namespace GovernorVotesCompErrors {
...
export class GovernorVotesComp extends ContractBase {
    constructor(
    ...
    Types: TGovernorVotesCompTypes;

    $meta = {
        "class": "./src/prebuilt/openzeppelin/GovernorVotesComp.ts"
    }
    ...
    $call () {
        return super.$call() as IGovernorVotesCompTxCaller;
    }
    $signed (): TOverrideReturns<IGovernorVotesCompTxCaller, Promise<{ signed: TEth.Hex, error?: Error & { data?: { type: string, params } } }>> {
        return super.$signed() as any;
    }
    $data (): IGovernorVotesCompTxData {
        return super.$data() as IGovernorVotesCompTxData;
    }
    $gas (): TOverrideReturns<IGovernorVotesCompTxCaller, Promise<{ gas?: bigint, price?: bigint, error?: Error & { data?: { type: string, params } } }>> {
        return super.$gas() as any;
    }

    onTransaction <TMethod extends keyof TGovernorVotesCompTypes['Methods']> (method: TMethod, options: Parameters<ContractBase['$onTransaction']>[0]): SubjectStream<{
        tx: TEth.Tx
        block: TEth.Block<TEth.Hex>
        calldata: {
            method: TMethod
            arguments: TGovernorVotesCompTypes['Methods'][TMethod]['arguments']
        }
    }> {
        options ??= {};
        options.filter ??= {};
...
export type TGovernorVotesCompTypes = {
    Events: {
        EIP712DomainChanged: {
            outputParams: {  },
            outputArgs:   [  ],
...
interface IGovernorVotesCompTxCaller {
...
interface IGovernorVotesCompTxData {
...
type TEvents = TGovernorVotesCompTypes['Events'];
