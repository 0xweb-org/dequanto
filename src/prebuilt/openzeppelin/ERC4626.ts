...
export class ERC4626 extends ContractBase {
    ...
    $meta = {
        "class": "./src/prebuilt/openzeppelin/ERC4626.ts"
    }
    ...
    async maxDeposit (input0: TAddress): Promise<bigint> {
        return this.$read(this.$getAbiItem('function', 'maxDeposit'), input0);
    }
    ...
    $call () {
        return super.$call() as IERC4626TxCaller;
    }
    $signed (): TOverrideReturns<IERC4626TxCaller, Promise<{ signed: TEth.Hex, error?: Error & { data?: { type: string, params } } }>> {
        return super.$signed() as any;
    }
    $data (): IERC4626TxData {
        return super.$data() as IERC4626TxData;
    }
    $gas (): TOverrideReturns<IERC4626TxCaller, Promise<{ gas?: bigint, price?: bigint, error?: Error & { data?: { type: string, params } } }>> {
        return super.$gas() as any;
    }

    onTransaction <TMethod extends keyof TERC4626Types['Methods']> (method: TMethod, options: Parameters<ContractBase['$onTransaction']>[0]): SubjectStream<{
        tx: TEth.Tx
        block: TEth.Block<TEth.Hex>
        calldata: {
            method: TMethod
            arguments: TERC4626Types['Methods'][TMethod]['arguments']
        }
    }> {
        options ??= {};
        options.filter ??= {};
...
type TEvents = TERC4626Types['Events'];
