...
export class IERC4906 extends ContractBase {
    ...
    Types: TIERC4906Types;

    $meta = {
        "class": "./src/prebuilt/openzeppelin/IERC4906.ts"
    }
    ...
    $signed (): TOverrideReturns<IIERC4906TxCaller, Promise<{ signed: TEth.Hex, error?: Error & { data?: { type: string, params } } }>> {
        return super.$signed() as any;
    }
    $data (): IIERC4906TxData {
        return super.$data() as IIERC4906TxData;
    }
    $gas (): TOverrideReturns<IIERC4906TxCaller, Promise<{ gas?: bigint, price?: bigint, error?: Error & { data?: { type: string, params } } }>> {
        return super.$gas() as any;
    }

    onTransaction <TMethod extends keyof TIERC4906Types['Methods']> (method: TMethod, options: Parameters<ContractBase['$onTransaction']>[0]): SubjectStream<{
        tx: TEth.Tx
        block: TEth.Block<TEth.Hex>
        calldata: {
            method: TMethod
            arguments: TIERC4906Types['Methods'][TMethod]['arguments']
        }
    }> {
        options ??= {};
        options.filter ??= {};
...
type TEvents = TIERC4906Types['Events'];
