...
export class ERC721 extends ContractBase {
    constructor(
    ...
    $meta = {
        "class": "./src/prebuilt/openzeppelin/ERC721.ts"
    }

    async $constructor (deployer: TSender, name_: string, symbol_: string): Promise<TxWriter> {
    ...
    $call () {
        return super.$call() as IERC721TxCaller;
    }
    $signed (): TOverrideReturns<IERC721TxCaller, Promise<{ signed: TEth.Hex, error?: Error & { data?: { type: string, params } } }>> {
        return super.$signed() as any;
    }
    $data (): IERC721TxData {
        return super.$data() as IERC721TxData;
    }
    $gas (): TOverrideReturns<IERC721TxCaller, Promise<{ gas?: bigint, price?: bigint, error?: Error & { data?: { type: string, params } } }>> {
        return super.$gas() as any;
    }

    onTransaction <TMethod extends keyof TERC721Types['Methods']> (method: TMethod, options: Parameters<ContractBase['$onTransaction']>[0]): SubjectStream<{
        tx: TEth.Tx
        block: TEth.Block<TEth.Hex>
        calldata: {
            method: TMethod
            arguments: TERC721Types['Methods'][TMethod]['arguments']
        }
    }> {
        options ??= {};
        options.filter ??= {};
...
type TEvents = TERC721Types['Events'];
