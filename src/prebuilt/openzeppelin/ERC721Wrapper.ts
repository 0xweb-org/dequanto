...
export class ERC721Wrapper extends ContractBase {
    constructor(
    ...
    Types: TERC721WrapperTypes;

    $meta = {
        "class": "./src/prebuilt/openzeppelin/ERC721Wrapper.ts"
    }
    ...
    // 0xcace6eb2
    async depositFor (sender: TSender, account: TAddress, tokenIds: bigint[]): Promise<TxWriter> {
        return this.$write(this.$getAbiItem('function', 'depositFor'), sender, account, tokenIds);
    }
    ...
    // 0x150b7a02
    async onERC721Received (sender: TSender, input0: TAddress, from: TAddress, tokenId: bigint, input3: TEth.Hex): Promise<TxWriter> {
        return this.$write(this.$getAbiItem('function', 'onERC721Received'), sender, input0, from, tokenId, input3);
    }
    ...
    // 0x6f307dc3
    async underlying (): Promise<TAddress> {
        return this.$read(this.$getAbiItem('function', 'underlying'));
    }

    // 0x7c1b126c
    async withdrawTo (sender: TSender, account: TAddress, tokenIds: bigint[]): Promise<TxWriter> {
        return this.$write(this.$getAbiItem('function', 'withdrawTo'), sender, account, tokenIds);
    }

    $call () {
        return super.$call() as IERC721WrapperTxCaller;
    }
    $signed (): TOverrideReturns<IERC721WrapperTxCaller, Promise<{ signed: TEth.Hex, error?: Error & { data?: { type: string, params } } }>> {
        return super.$signed() as any;
    }
    $data (): IERC721WrapperTxData {
        return super.$data() as IERC721WrapperTxData;
    }
    $gas (): TOverrideReturns<IERC721WrapperTxCaller, Promise<{ gas?: bigint, price?: bigint, error?: Error & { data?: { type: string, params } } }>> {
        return super.$gas() as any;
    }

    onTransaction <TMethod extends keyof TERC721WrapperTypes['Methods']> (method: TMethod, options: Parameters<ContractBase['$onTransaction']>[0]): SubjectStream<{
        tx: TEth.Tx
        block: TEth.Block<TEth.Hex>
        calldata: {
            method: TMethod
            arguments: TERC721WrapperTypes['Methods'][TMethod]['arguments']
        }
    }> {
        options ??= {};
        options.filter ??= {};
...
export type TERC721WrapperTypes = {
    Events: {
        Approval: {
    ...
    Methods: {
        ...
        depositFor: {
          method: "depositFor"
          arguments: [ account: TAddress, tokenIds: bigint[] ]
        }
        getApproved: {
          method: "getApproved"
        ...
        onERC721Received: {
          ...
          arguments: [ input0: TAddress, from: TAddress, tokenId: bigint, input3: TEth.Hex ]
        }
        ownerOf: {
          method: "ownerOf"
          arguments: [ tokenId: bigint ]
...
interface IERC721WrapperTxCaller {
    approve (sender: TSender, to: TAddress, tokenId: bigint): Promise<{ error?: Error & { data?: { type: string, params } }, result? }>
    depositFor (sender: TSender, account: TAddress, tokenIds: bigint[]): Promise<{ error?: Error & { data?: { type: string, params } }, result? }>
    onERC721Received (sender: TSender, input0: TAddress, from: TAddress, tokenId: bigint, input3: TEth.Hex): Promise<{ error?: Error & { data?: { type: string, params } }, result? }>
    ...
    withdrawTo (sender: TSender, account: TAddress, tokenIds: bigint[]): Promise<{ error?: Error & { data?: { type: string, params } }, result? }>
}


interface IERC721WrapperTxData {
    approve (sender: TSender, to: TAddress, tokenId: bigint): Promise<TEth.TxLike>
    depositFor (sender: TSender, account: TAddress, tokenIds: bigint[]): Promise<TEth.TxLike>
    onERC721Received (sender: TSender, input0: TAddress, from: TAddress, tokenId: bigint, input3: TEth.Hex): Promise<TEth.TxLike>
    ...
    withdrawTo (sender: TSender, account: TAddress, tokenIds: bigint[]): Promise<TEth.TxLike>
}


type TEvents = TERC721WrapperTypes['Events'];
