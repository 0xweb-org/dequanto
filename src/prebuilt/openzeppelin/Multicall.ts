...
export class Multicall extends ContractBase {
    ...
    Types: TMulticallTypes;

    $meta = {
        "class": "./src/prebuilt/openzeppelin/Multicall.ts"
    }
    ...
    $signed (): TOverrideReturns<IMulticallTxCaller, Promise<{ signed: TEth.Hex, error?: Error & { data?: { type: string, params } } }>> {
        return super.$signed() as any;
    }
    $data (): IMulticallTxData {
        return super.$data() as IMulticallTxData;
    }
    $gas (): TOverrideReturns<IMulticallTxCaller, Promise<{ gas?: bigint, price?: bigint, error?: Error & { data?: { type: string, params } } }>> {
        return super.$gas() as any;
    }

    onTransaction <TMethod extends keyof TMulticallTypes['Methods']> (method: TMethod, options: Parameters<ContractBase['$onTransaction']>[0]): SubjectStream<{
        tx: TEth.Tx
        block: TEth.Block<TEth.Hex>
        calldata: {
            method: TMethod
            arguments: TMulticallTypes['Methods'][TMethod]['arguments']
        }
    }> {
        options ??= {};
        options.filter ??= {};
...
type TEvents = TMulticallTypes['Events'];
