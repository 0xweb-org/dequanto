...
export class IERC2612 extends ContractBase {
    constructor(
    ...
    Types: TIERC2612Types;

    $meta = {
        "class": "./src/prebuilt/openzeppelin/IERC2612.ts"
    }
    ...
    $call () {
        return super.$call() as IIERC2612TxCaller;
    }
    $signed (): TOverrideReturns<IIERC2612TxCaller, Promise<{ signed: TEth.Hex, error?: Error & { data?: { type: string, params } } }>> {
        return super.$signed() as any;
    }
    $data (): IIERC2612TxData {
        return super.$data() as IIERC2612TxData;
    }
    $gas (): TOverrideReturns<IIERC2612TxCaller, Promise<{ gas?: bigint, price?: bigint, error?: Error & { data?: { type: string, params } } }>> {
        return super.$gas() as any;
    }

    onTransaction <TMethod extends keyof TIERC2612Types['Methods']> (method: TMethod, options: Parameters<ContractBase['$onTransaction']>[0]): SubjectStream<{
        tx: TEth.Tx
        block: TEth.Block<TEth.Hex>
        calldata: {
            method: TMethod
            arguments: TIERC2612Types['Methods'][TMethod]['arguments']
        }
    }> {
        options ??= {};
        options.filter ??= {};
...
type TEvents = TIERC2612Types['Events'];
