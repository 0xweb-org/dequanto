...
export class IERC721 extends ContractBase {
    ...
    $meta = {
        "class": "./src/prebuilt/openzeppelin/IERC721.ts"
    }
    ...
    $signed (): TOverrideReturns<IIERC721TxCaller, Promise<{ signed: TEth.Hex, error?: Error & { data?: { type: string, params } } }>> {
        return super.$signed() as any;
    }
    $data (): IIERC721TxData {
        return super.$data() as IIERC721TxData;
    }
    $gas (): TOverrideReturns<IIERC721TxCaller, Promise<{ gas?: bigint, price?: bigint, error?: Error & { data?: { type: string, params } } }>> {
        return super.$gas() as any;
    }

    onTransaction <TMethod extends keyof TIERC721Types['Methods']> (method: TMethod, options: Parameters<ContractBase['$onTransaction']>[0]): SubjectStream<{
        tx: TEth.Tx
        block: TEth.Block<TEth.Hex>
        calldata: {
            method: TMethod
            arguments: TIERC721Types['Methods'][TMethod]['arguments']
        }
    }> {
        options ??= {};
        options.filter ??= {};
...
type TEvents = TIERC721Types['Events'];
