...
export class ERC20Wrapper extends ContractBase {
    constructor(
    ...
    Types: TERC20WrapperTypes;

    $meta = {
        "class": "./src/prebuilt/openzeppelin/ERC20Wrapper.ts"
    }
    ...
    // 0x2f4f21e2
    async depositFor (sender: TSender, account: TAddress, amount: bigint): Promise<TxWriter> {
        return this.$write(this.$getAbiItem('function', 'depositFor'), sender, account, amount);
    }
    ...
    // 0x205c2878
    async withdrawTo (sender: TSender, account: TAddress, amount: bigint): Promise<TxWriter> {
        return this.$write(this.$getAbiItem('function', 'withdrawTo'), sender, account, amount);
    }

    $call () {
        return super.$call() as IERC20WrapperTxCaller;
    }
    $signed (): TOverrideReturns<IERC20WrapperTxCaller, Promise<{ signed: TEth.Hex, error?: Error & { data?: { type: string, params } } }>> {
        return super.$signed() as any;
    }
    $data (): IERC20WrapperTxData {
        return super.$data() as IERC20WrapperTxData;
    }
    $gas (): TOverrideReturns<IERC20WrapperTxCaller, Promise<{ gas?: bigint, price?: bigint, error?: Error & { data?: { type: string, params } } }>> {
        return super.$gas() as any;
    }

    onTransaction <TMethod extends keyof TERC20WrapperTypes['Methods']> (method: TMethod, options: Parameters<ContractBase['$onTransaction']>[0]): SubjectStream<{
        tx: TEth.Tx
        block: TEth.Block<TEth.Hex>
        calldata: {
            method: TMethod
            arguments: TERC20WrapperTypes['Methods'][TMethod]['arguments']
        }
    }> {
        options ??= {};
        options.filter ??= {};
...
export type TERC20WrapperTypes = {
    Events: {
        Approval: {
...
interface IERC20WrapperTxCaller {
    ...
    depositFor (sender: TSender, account: TAddress, amount: bigint): Promise<{ error?: Error & { data?: { type: string, params } }, result? }>
    ...
    withdrawTo (sender: TSender, account: TAddress, amount: bigint): Promise<{ error?: Error & { data?: { type: string, params } }, result? }>
}


interface IERC20WrapperTxData {
    ...
    depositFor (sender: TSender, account: TAddress, amount: bigint): Promise<TEth.TxLike>
    ...
    withdrawTo (sender: TSender, account: TAddress, amount: bigint): Promise<TEth.TxLike>
}


type TEvents = TERC20WrapperTypes['Events'];
