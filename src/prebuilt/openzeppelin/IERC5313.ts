...
export class IERC5313 extends ContractBase {
    ...
    Types: TIERC5313Types;

    $meta = {
        "class": "./src/prebuilt/openzeppelin/IERC5313.ts"
    }
    ...
    $signed (): TOverrideReturns<IIERC5313TxCaller, Promise<{ signed: TEth.Hex, error?: Error & { data?: { type: string, params } } }>> {
    ...
    onTransaction <TMethod extends keyof TIERC5313Types['Methods']> (method: TMethod, options: Parameters<ContractBase['$onTransaction']>[0]): SubjectStream<{
        tx: TEth.Tx
        block: TEth.Block<TEth.Hex>
        calldata: {
            method: TMethod
            arguments: TIERC5313Types['Methods'][TMethod]['arguments']
        }
    }> {
        options ??= {};
        options.filter ??= {};
...
type TEvents = TIERC5313Types['Events'];
