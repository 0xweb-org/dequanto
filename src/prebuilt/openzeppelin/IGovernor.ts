...
export class IGovernor extends ContractBase {
    constructor(
    ...
    Types: TIGovernorTypes;

    $meta = {
        "class": "./src/prebuilt/openzeppelin/IGovernor.ts"
    }
    ...
    $call () {
        return super.$call() as IIGovernorTxCaller;
    }
    $signed (): TOverrideReturns<IIGovernorTxCaller, Promise<{ signed: TEth.Hex, error?: Error & { data?: { type: string, params } } }>> {
        return super.$signed() as any;
    }
    $data (): IIGovernorTxData {
        return super.$data() as IIGovernorTxData;
    }
    $gas (): TOverrideReturns<IIGovernorTxCaller, Promise<{ gas?: bigint, price?: bigint, error?: Error & { data?: { type: string, params } } }>> {
        return super.$gas() as any;
    }

    onTransaction <TMethod extends keyof TIGovernorTypes['Methods']> (method: TMethod, options: Parameters<ContractBase['$onTransaction']>[0]): SubjectStream<{
        tx: TEth.Tx
        block: TEth.Block<TEth.Hex>
        calldata: {
            method: TMethod
            arguments: TIGovernorTypes['Methods'][TMethod]['arguments']
        }
    }> {
        options ??= {};
        options.filter ??= {};
...
type TEvents = TIGovernorTypes['Events'];
