...
export class AccessControl extends ContractBase {
    ...
    Types: TAccessControlTypes;

    $meta = {
        "class": "./src/prebuilt/openzeppelin/AccessControl.ts"
    }
    ...
    onTransaction <TMethod extends keyof TAccessControlTypes['Methods']> (method: TMethod, options: Parameters<ContractBase['$onTransaction']>[0]): SubjectStream<{
        tx: TEth.Tx
        block: TEth.Block<TEth.Hex>
        calldata: {
            method: TMethod
            arguments: TAccessControlTypes['Methods'][TMethod]['arguments']
        }
    }> {
        options ??= {};
        options.filter ??= {};
...
export type TAccessControlTypes = {
    Events: {
        RoleAdminChanged: {
...
type TEvents = TAccessControlTypes['Events'];
