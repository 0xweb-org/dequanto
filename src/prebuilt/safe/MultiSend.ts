...
export class MultiSend extends ContractBase {
    ...
    Types: TMultiSendTypes;

    $meta = {
        "class": "./src/prebuilt/safe/MultiSend.ts"
    }
    ...
    onTransaction <TMethod extends keyof TMultiSendTypes['Methods']> (method: TMethod, options: Parameters<ContractBase['$onTransaction']>[0]): SubjectStream<{
        tx: TEth.Tx
        block: TEth.Block<TEth.Hex>
        calldata: {
            method: TMethod
            arguments: TMultiSendTypes['Methods'][TMethod]['arguments']
        }
    }> {
        options ??= {};
        options.filter ??= {};
...
type TEvents = TMultiSendTypes['Events'];
