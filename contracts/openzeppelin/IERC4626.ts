...
export class IERC4626 extends ContractBase {
    ...
    async maxDeposit (receiver: TAddress): Promise<bigint> {
        return this.$read('function maxDeposit(address) returns uint256', receiver);
    }

    // 0xc63d75b6
    async maxMint (receiver: TAddress): Promise<bigint> {
        return this.$read('function maxMint(address) returns uint256', receiver);
    }
    ...
    onDeposit (fn: (event: EventLog, caller: TAddress, owner: TAddress, assets: bigint, shares: bigint) => void): ClientEventsStream<any> {
    ...
    onWithdraw (fn: (event: EventLog, caller: TAddress, receiver: TAddress, owner: TAddress, assets: bigint, shares: bigint) => void): ClientEventsStream<any> {
