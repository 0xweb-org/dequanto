...
export class GovernorCompatibilityBravo extends ContractBase {
    ...
    async cancel (sender: TSender, proposalId: bigint): Promise<TxWriter> {
        return this.$write(this.$getAbiItem('function', 'cancel'), sender, proposalId);
    }
    ...
    async execute (sender: TSender, proposalId: bigint): Promise<TxWriter> {
        return this.$write(this.$getAbiItem('function', 'execute'), sender, proposalId);
    }

    // 0x328dd982
    async getActions (proposalId: bigint): Promise<{ targets: TAddress[], values: bigint[], signatures: string[], calldatas: TBufferLike[] }> {
        return this.$read('function getActions(uint256) returns (address[],uint256[],string[],bytes[])', proposalId);
    }

    // 0xe23a9a52
    async getReceipt (proposalId: bigint, voter: TAddress): Promise<{ hasVoted: boolean, support: number, votes: bigint }> {
        return this.$read('function getReceipt(uint256, address) returns [bool,uint8,uint96]', proposalId, voter);
    }
    ...
    async proposals (proposalId: bigint): Promise<{ id: bigint, proposer: TAddress, eta: bigint, startBlock: bigint, endBlock: bigint, forVotes: bigint, againstVotes: bigint, abstainVotes: bigint, canceled: boolean, executed: boolean }> {
        return this.$read('function proposals(uint256) returns (uint256,address,uint256,uint256,uint256,uint256,uint256,uint256,bool,bool)', proposalId);
    }
    ...
    async propose (sender: TSender, targets: TAddress[], values: bigint[], signatures: string[], calldatas: TBufferLike[], description: string): Promise<TxWriter> {
        return this.$write(this.$getAbiItem('function', 'propose'), sender, targets, values, signatures, calldatas, description);
    }
    ...
    async quorumVotes (): Promise<bigint> {
        return this.$read('function quorumVotes() returns uint256');
    }
    ...
    onProposalQueued (fn: (event: EventLog, proposalId: bigint, eta: bigint) => void): ClientEventsStream<any> {
        return this.$on('ProposalQueued', fn);
    }
